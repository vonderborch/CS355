...
def _GET(x,linker):
    ...
    if found == True:
        if len(temp) > 1:
            print("interpreting...new link: " + str(len(dictionary) - 1))
            InterpretorLoop(temp,len(dictionary) - 1)
        else:
            z = convert(temp[0])
    ...
    while loopRunning == True:
        ...
        if t == 'and':
            ...
            if t == 'if': # if this is only an if statement, we're done already!
                if c == True:
                    InterpretorLoop(logicTrue, linker)
                    return
                ...
                if t == 'ifelse' and c == False:
                    InterpretorLoop(logicFalse, linker)
                    return
